...
# Add User -------------------------------------------------------------------------------------------------------------
RUN useradd -m ohmyfi && echo 'ohmyfi:ohmyfi' | chpasswd && usermod -aG sudo ohmyfi && usermod -s /bin/bash ohmyfi
# Dependencies ---------------------------------------------------------------------------------------------------------
RUN apt-get update -y \
    && apt-get install -y sudo nano zsh git curl wget unzip golang-go systemctl \
...
RUN a2enmod rewrite ssl \
    ...
    && sed -i "s|www-data|ohmyfi|g" /etc/apache2/envvars \
    && openssl req -x509 -nodes -days 365 -newkey rsa:4096 \
        -keyout /etc/ssl/private/apache-selfsigned.key \
        -out /etc/ssl/certs/apache-selfsigned.crt \
        -subj "/C=US/ST=State/L=City/O=ohmyfi-Packages/OU=ohmyfi/CN=localhost/emailAddress=root@dev.com" \
...
RUN echo 'export NVM_DIR="$HOME/.nvm"' >> /home/ohmyfi/.zshrc \
    && echo '[ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"' >> /home/ohmyfi/.zshrc \
    && echo '[ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"' >> /home/ohmyfi/.zshrc \
    && echo 'alias ls="logo-ls"' >> /home/ohmyfi/.zshrc \
    && source /home/ohmyfi/.zshrc
# Copy Data ------------------------------------------------------------------------------------------------------------
COPY . .
# Permissions ----------------------------------------------------------------------------------------------------------
RUN chown -R ohmyfi:ohmyfi /var/www/html \
